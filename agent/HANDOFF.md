...
2. **Did execution become measurably faster in compiled native mode?**
   ...
   - *Limitations:* Wall-clock timing includes process startup and memory initialization; short-run noise (~±30%) affects small speedup ratios.
...
4. **Which specific operational phase (discovery, read, hash, grouping/output) exhibited performance variance?**
   - *Answer:* Under the standalone cumulative stage-probe model, performance variance across corpus types was concentrated in **Size Filter ($O(N^2)$)** in large-file corpora (e.g. C1: approximately 86% of total probe time, 2,192.2 ms out of 2,540.6 ms total). In candidate-dense corpora (e.g. C2), pairwise size filtering also dominated valid probe time (181.2 ms). Individual micro-stage durations below the ~10 ms process invocation noise floor (such as read/hash in C2/C7 or grouping in C2/C6) yielded non-positive deltas marked as below noise floor (`N/A*`) rather than zero-clipped, and cannot be reliably separated without internal runtime instrumentation.
   - *Evidence Grade:* **B**
...
6. **At what workload dimensions (file count, file size, candidate density) did scaling plateau?**
   - *Answer:* Scaling plateaued primarily with file count due to the $O(N^2)$ pairwise size filtering algorithm in `scan.j2`. At 500 files (C1), size filtering consumed approximately 86% of execution time under the standalone probe model.
   - *Evidence Grade:* **A**
